...
public class CastingDepotItemHandler extends SnapshotParticipant<Unit> implements Storage<ItemVariant> {
	...
	public long insert(ItemVariant resource, long maxAmount, TransactionContext transaction) {
		if (!behaviour.getHeldItemStack().isEmpty() && !behaviour.canMergeItems()) return 0;
		if (!behaviour.isOutputEmpty() && !behaviour.canMergeItems()) return 0;
		...
		if (!behaviour.isItemValid(stack)) return 0;
	...
	public long extract(ItemVariant resource, long maxAmount, TransactionContext transaction) {
		...
		if (extracted == maxAmount) return extracted;
	...
	public long extractFromMain(ItemVariant resource, long maxAmount, TransactionContext transaction) {
		...
		int
				toExtract =
				Math.min(ItemHelper.truncateLong(maxAmount), Math.min(stack.getCount(), behaviour.maxStackSize.get()));
		stack = stack.copy();
		stack.shrink(toExtract);
		if (stack.isEmpty()) stack = ItemStack.EMPTY;
		behaviour.snapshotParticipant.updateSnapshots(transaction);
		behaviour.heldItem.stack = stack;
		if (stack.isEmpty()) behaviour.heldItem = null;
		return toExtract;
	}

	@Override
	public Iterator<StorageView<ItemVariant>> iterator() {
		return Iterators.concat(Iterators.singletonIterator(new CastingDepotItemHandler.MainSlotView()),
				behaviour.processingOutputBuffer.iterator());
	}
	...
	public class MainSlotView implements StorageView<ItemVariant> {
		...
		public long getCapacity() {
			...
			return stack.isEmpty() ?
				   behaviour.maxStackSize.get() :
				   Math.min(stack.getMaxStackSize(), behaviour.maxStackSize.get());
		}

		public ItemStack getStack() {
			TransportedItemStack held = behaviour.heldItem;
			if (held == null || held.stack == null || held.stack.isEmpty()) return ItemStack.EMPTY;
			return held.stack;
		}
	}
