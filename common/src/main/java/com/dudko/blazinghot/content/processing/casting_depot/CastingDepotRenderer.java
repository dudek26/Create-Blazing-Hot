...
import com.dudko.blazinghot.mixin.accessor.DepotBehaviourAccessor;
...
public class CastingDepotRenderer extends SafeBlockEntityRenderer<CastingDepotBlockEntity> {
	...
	public static void renderItemsOf(SmartBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay, DepotBehaviour depotBehaviour) {
		...
		TransportedItemStack transported = duck(depotBehaviour).getHeldItem();
		...
		if (transported != null) duck(depotBehaviour).getIncoming().add(transported);

		// Render main items
		for (TransportedItemStack tis : duck(depotBehaviour).getIncoming()) {
			ms.pushPose();
			msr.nudge(0);
		...
		if (transported != null) duck(depotBehaviour).getIncoming().remove(transported);

		// Render output items
		for (int i = 0; i < duck(depotBehaviour).getProcessingOutputBuffer().getSlotCount(); i++) {
			ItemStack stack = duck(depotBehaviour).getProcessingOutputBuffer().getStackInSlot(i);
			...
			renderItem(be.getLevel(), ms, buffer, light, overlay, stack, angle, r, itemPosition);
			ms.popPose();
		}
	...
	public static void renderItem(Level level, PoseStack ms, MultiBufferSource buffer, int light, int overlay, ItemStack itemStack, int angle, RandomSource r, Vec3 itemPosition) {
	...
	private static DepotBehaviourAccessor duck(DepotBehaviour depotBehaviour) {
		return (DepotBehaviourAccessor) depotBehaviour;
	}
